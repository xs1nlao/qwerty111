...
Для врача: Детальный анализ с указанием источников, расчетом процента соответствия стандартам и разбором каждой линии терапии.
...
** Сбор метрик: Система автоматически собирает статистику использования, времени ответа и распределения c расчетом процента соответствия стандартам

Архитектура решения

** Frontend: React. Отвечает за пользовательский интерфейс, маршрутизацию, отправку запросов на бэкенд и отображение результатов.
** Backend: Flask. Обрабатывает все запросы от фронтенда, управляет бизнес-логикой.
** AI Сервис: Центральный модуль для взаимодействия с DeepSeek API. Отвечает за: определение типа рака, извлечение линий терапии, интеллектуальный анализ недостающей информации.
** Подсчет: Вычисляет процент совпадения с клиническими рекомендациями
** Загрузчик базы знаний: Парсит и индексирует все JSON-файлы с клиническими рекомендациями из папки data/, создавая удобный кэш для быстрого доступа
** Менеджер пациентов: Отвечает за CRUD-операции с данными пациентов и историей их проверок, сохраняя все в patients_db.json.
** Модуль маммограмм: Инкапсулирует логику AI-модели для анализа изображений. В текущей версии работает в демо-режиме, но архитектура позволяет легко подключить реальную модель.
** Сборщик метрик: Собирает статистику работы системы для последующего анализа
...
2.  Настройка бэкенда:
   Перейдите в папку с бэкендом (если она отдельная, иначе в корень).
...
DEEPSEEK_API_KEY="ваш_ключ_сюда"
    ...
    *   Важно: Поместите файлы клинических рекомендаций (JSON) в папку data/. Пример структуры файлов описан в data/README.md.
